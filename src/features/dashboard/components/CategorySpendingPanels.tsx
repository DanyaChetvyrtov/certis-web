...
import {CategorySpendingOverTimeChart} from './CategorySpendingOverTimeChart'
import './CategorySpendingPanels.css'
...
type CategorySpendingPanelsProps = {
    ...
    spendingOverTimeLoadState: DashboardCategorySpendingOverTimeLoadState
    onRetrySpendingOverTime: () => void
}
...
export function CategorySpendingPanels({
    ...
    onRetry,
    spendingOverTime,
    spendingOverTimeLoadState,
    onRetrySpendingOverTime,
    ...
    const slices = analytics
        ...
        : []
    const displaySpendingOverTime = spendingOverTime
        ? {
            ...spendingOverTime,
            series: spendingOverTime.series.map((series) => (
                series.categoryId === null
                    ? {
                        ...series,
                        categoryName: t('dashboard.categoryAnalytics.other'),
                    }
                    : series
            )),
        }
        : null
    const spendingOverTimeTotal = positiveAmount(
        displaySpendingOverTime?.totalSum ?? 0,
    )
    ...
    return (
        ...
        >
            <article className="dashboard-panel dashboard-category-panel category-over-time-panel">
                <header className="dashboard-category-header category-over-time-header">
                    <div>
                    ...
                    {spendingOverTimeLoadState === 'ready'
                        && displaySpendingOverTime
                        && spendingOverTimeTotal > 0 && (
                        <ul className="category-over-time-legend">
                            {displaySpendingOverTime.series.map((series) => (
                                <li key={series.categoryId ?? 'other'}>
                                    <i style={{backgroundColor: series.categoryColor || OTHER_COLOR}}/>
                ...
                {(spendingOverTimeLoadState === 'idle'
                    || spendingOverTimeLoadState === 'loading') && (
                    <LoadingIndicator
                        label={t('dashboard.categoryAnalytics.loadingHistory')}
                        layout="panel"
                        showLabel
                        size="medium"
                    />
                ...
                {spendingOverTimeLoadState === 'error' && (
                    <div className="category-spending-state error" role="alert">
                        <Icon name="alert"/>
                        <p>{t('dashboard.categoryAnalytics.historyError')}</p>
                        <button type="button" onClick={onRetrySpendingOverTime}>
                ...
                {spendingOverTimeLoadState === 'ready'
                    ...
                    <div className="category-spending-state empty">
                        <Icon name="trend-up"/>
                        <strong>{t('dashboard.categoryAnalytics.noHistory')}</strong>
                        <p>{t('dashboard.categoryAnalytics.noHistoryDescription')}</p>
                    </div>
                )}
                ...
                {spendingOverTimeLoadState === 'ready'
                    && displaySpendingOverTime
                    && spendingOverTimeTotal > 0 && (
                    <div className="category-over-time-content">
                        <CategorySpendingOverTimeChart
                            analytics={displaySpendingOverTime}
                            currency={currency}
                            description={t('dashboard.categoryAnalytics.historyChartLabel', {
                                currency,
                            })}
                        />
                    </div>
